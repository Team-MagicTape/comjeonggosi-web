...
import TodayQuestionItemSkeleton from "./TodayQuestionItemSkeleton";
...
const TodayQuestionItem = ({ data, category, isLoading }: Props) => {
  if (isLoading) {
    return <TodayQuestionItemSkeleton />;
  }
  return (
    <CustomLink
      href={`/questions/${data.id}`}
      className="w-full h-full bg-white rounded-[16px] p-7"
    >
      ...
      <div className="line-clamp-2 text-[#333]">{data.answer}</div>
    </CustomLink>
  );
};
