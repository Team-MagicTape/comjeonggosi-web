    ...
    <div className="relative p-6 rounded-item border border-border">
      <div className="flex items-center gap-3 mb-1">
        <div className="flex items-center gap-2">
          ...
          <div className={`px-3 py-1 rounded-full text-xs ${data.isCorrected ? "bg-green-100 text-green-600" : "bg-red-200 text-red-600"}`}>
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 px-3 py-2 rounded-lg font-medium border border-border flex items-center gap-4 flex-wrap">
            <p className={`${!data.isCorrected ? "line-through [text-decoration-color:red] [text-decoration-style:double]" : "text-green-600 font-semibold"} break-keep`}>{data.userAnswer}</p>
            {!data.isCorrected && <p className="break-keep text-green-600 font-semibold">{data.quiz.answer}</p>}
          </div>
        </div>
      </div>
